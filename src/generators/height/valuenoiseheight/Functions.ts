...
export function valueNoiseMap(size: number, seed: number = 0, scaleH: number = 1, scaleV: number = 0.01, rawScaleV: number = 1, rawShift: number = 0 ,exponent: number = 1, octaves: number = 3, lacunarity: number = 2, persistence: number = 0.5): Float32Array {
...
function octave(x: number, y: number, ptable: number[], octaves: number, lacunarity: number, persistence: number, rawScaleV: number, rawShift: number, exponent: number) {
  ...
  for (let i = 0; i <= octaves; i++) {
    const raw: number = valueNoise(x * frequency, y * frequency, ptable);
...
function valueNoise(x: number, y: number, ptable: number[]): number {
    ...
    // calculate the values of the 4 closest grid points
    const n00: number = ptable[(ptable[xi % 512] + yi) % 512]/512;
    const n01: number = ptable[(ptable[xi % 512] + yi + 1) % 512]/512;
    const n11: number = ptable[(ptable[(xi + 1) % 512] + yi + 1) % 512]/512;
    const n10: number = ptable[(ptable[(xi + 1) % 512] + yi) % 512]/512;
    ...
    const x1: number = lerp(n00, n10, xg);
    const x2: number = lerp(n01, n11, xg);

    return lerp(x1, x2, yg) + 1;
}
