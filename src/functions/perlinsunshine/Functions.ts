...
export function perlinMap(size: number, seed: number = 0, scaleH: number = 1, scaleV: number = 0.01, rawScaleV: number = 1, rawShift: number = 0 ,exponent: number = 1, octaves: number = 3, lacunarity: number = 2, persistence: number = 0.5, lacunarityScale: number = 1, persistenceScale: number = 1): Float32Array {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      data[x * size + y] = ( octave(x / scaleH, y / scaleH, ptable, octaves, lacunarity, persistence, lacunarityScale, persistenceScale, rawScaleV, rawShift, exponent)) * scaleV;
    }
  }
  return data;
}

function octave(x: number, y: number, ptable: number[], octaves: number, lacunarity: number, persistence: number, lacunarityScale: number, persistenceScale: number, rawScaleV: number, rawShift: number, exponent: number) {
